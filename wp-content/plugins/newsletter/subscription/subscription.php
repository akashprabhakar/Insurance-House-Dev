...
class NewsletterSubscription extends NewsletterModule {
    ...
    function get_subscription_form($referrer = null, $action = null, $attrs = array()) {
        ...
        $buffer .= '<div class="newsletter newsletter-subscription input-group">' . "\n";
        if (empty($action)) {
        ...
        if ($options_profile['name_status'] == 2) {
            $buffer .= "<!-- first name -->\n";
            $buffer .= "\n\t" . '' . $options_profile['name'] . '' . "\n\t" . '<td><input class="newsletter-firstname" type="text" name="nn" size="30"' . ($options_profile['name_rules'] == 1 ? 'required' : '') . '' . "\n" . '' . "\n\n";
        }
    ...
    function get_subscription_form_html5($referrer = null, $action = null) {
        ...
        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            $buffer .= "\t\t" . '<input class="newsletter-submit-image" type="image" src="' . INC_URL_IMG . DS . 'instagrame_icon.png'. '">' . "\n\t";
        } else {
            $buffer .= "\t\t" . '<input class="newsletter-submit" type="submit" src="' . INC_URL_IMG . DS . 'instagrame_icon.png'. '" value="' . $options_profile['subscribe'] . '">' . "\n\t";
        }
