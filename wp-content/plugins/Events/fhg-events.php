...
function capm_functions_css() {
	wp_enqueue_style('capm_functions_css', get_bloginfo('template_directory') . '/includes/css/tf-functions.css');
}
...
function displayEventsAll() {
  ...
  if ($arg_post_data) {
    ...
    foreach ($arg_post_data as $post_data) {
      ...
      $sd = $custom["fhg_events_startdate"][0];
      $ed = $custom["fhg_events_startdate"][0];
      ...
      if (strstr($url, '/ar/') !== false) { //check if '?' is present
        $location = $custom['my_meta_box_text_events_ar'][0];
      } else {
        $location = $custom['my_meta_box_text_events'][0];
      }
      $length = str_word_count($post_name);

      if ($length >= 10) {
        $total_words = explode(" ", $post_name);
        $post_name = implode(" ", array_splice($total_words, 0, 15));

        $url = $_SERVER['REQUEST_URI'];
        if (strstr($url, '/ar/') !== false) { //check if '?' is present
          $post_name .= '...<a href="' . $customurl . '">اقرأ المزيد</a>';
        } else {
          $post_name .= '...<a href="' . $customurl . '">Read More</a>';
        }
      } else {
        $url = $_SERVER['REQUEST_URI'];
        if (strstr($url, '/ar/') !== false) { //check if '?' is present

          $post_name = $post_name . '...<a href="' . $customurl . '">اقرأ المزيد</a>';
        } else {
          $post_name = $post_name . '...<a href="' . $customurl . '">Read More</a>';
        }
      }
      ...
      if ($counter == 0) {
        ...
        $details .= '<div class="venueDate"><span>'.$date.':</span> <span>'. $gmts.'</span></div>';
    ...
    $details .= '</ul></div>';
            ...
            $details .= eventspagination(totalPostsEvents());
        echo $str .= $details;
        //echo $str = $cat_name;
    //echo $str = $cat_name;
  } else {
