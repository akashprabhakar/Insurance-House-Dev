...
function oQeyImageDetails(){
   ...
   $resp['arabic_description']             .= $image->arabic_description;
   $resp['arabic_comments']        .= $image->arabic_comments;
   $resp['splash']          .= $splash;
   $resp['splashexclusive'] .= $splashexclusive;
   $resp['link']            .= trim($image->img_link);
   $resp['arabiclink']      .= trim($image->img_arabiclink);
    $resp['youtube_url']      .= trim($image->youtube_url);
     $resp['upload_video']      .= trim($image->upload_video);
   echo json_encode($resp);
...
function oQeyUpdateImageDetails(){
   ...
   $wpdb->query( $wpdb->prepare("UPDATE $oqey_images SET alt = %s, comments = %s, img_link = %s,img_arabiclink = %s, arabic_description =%s, arabic_comments = %s, youtube_url = %s, upload_video = %s WHERE id = %d ", stripslashes_deep(urldecode($_POST['alt'])), stripslashes_deep(urldecode($_POST['comments'])), stripslashes_deep(urldecode($_POST['oqey_image_link'])), stripslashes_deep(urldecode($_POST['oqey_image_arabiclink'])),stripslashes_deep(urldecode($_POST['oqey_arabic_description'])),stripslashes_deep(urldecode($_POST['oqey_arabic_comments'])),stripslashes_deep(urldecode($_POST['youtube_url'])),stripslashes_deep(urldecode($_POST['upload_video'])), $imgId ) ); 
...
function oQeyEditVideoTitle(){
  ...
  $lang = $_POST['lang'];
   ...
   /* if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT * FROM $oqey_video_desc WHERE wp_title = %s AND wp_video_desc_id != %d ",$newtitle, $id))){ 
        echo "if";
        $title  = $newtitle;
      
      }else{
    echo "else";
        $sql    = $wpdb->get_row($wpdb->prepare("SELECT * FROM $oqey_video_desc WHERE wp_title = %s AND wp_video_desc_id != %d ",$newtitle, $id));
        $title  = $sql->title;  
    
    } */
  
  //echo "UPDATE $oqey_video_desc SET wp_title = '$newtitle' WHERE wp_video_desc_id = $id";
  
   $wpdb->query( $wpdb->prepare("UPDATE $oqey_video_desc SET wp_title = %s WHERE wp_video_desc_id = %d ", $newtitle,$id) );
    
    echo stripslashes_deep($newtitle);
...
function oQeyEditVideoDescription(){
  ...
  $oqey_video_desc = $wpdb->prefix . "video_desc";
    ...
    $wpdb->query( $wpdb->prepare("UPDATE $oqey_video_desc SET wp_desc = %s WHERE wp_video_desc_id = %d ", $desc, $id) );
