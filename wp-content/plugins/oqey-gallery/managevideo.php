...
if(isset($_REQUEST['extvideoname']) && isset($_REQUEST['savevideo'])){
 ...
 if(isset($_REQUEST['saveYoutTubevideo'])){
 
	$linkurl = $_REQUEST['youtubevideoname'];
	$title = "Youtube embed code";
	
	 if( !empty($_REQUEST['youtubevideoname']) ){
	 
	  $sql = $wpdb->query( $wpdb->prepare( "INSERT INTO $oqey_video (title, video_link, type) 
                                                                        VALUES ( %s, %s, %s)",
																				$title,
                                                                                $_REQUEST['youtubevideoname'],
																				'youtube'
                                                    )
                                    );
	  if($sql){
	 $lastid = $wpdb->insert_id;    
		foreach($language as $lang){             
			$sql = $wpdb->query( $wpdb->prepare( "INSERT INTO $oqey_video_desc (wp_title,video_id, lang) 
                                                                        VALUES ( %s, %d, %s )",
																				$title,
                                                                                $lastid,
																				$lang
     ...
     $mesaj = '<div class="wrap">
               <div class="updated below-h2" id="message">
               <p>'.__('added youtube embed code.', 'oqey-gallery').'</p>
               </div>
               </div>';
     }else{
...
<table width="880" border="0" cellspacing="0" cellpadding="0" style="margin:10px;">
  ...
  <tr>
	...
	<div class="postbox" id="youtube_video" style="width:900px;">

<form id="addyoutubevideofile" name="addyoutubevideofile" method="post" action="<?php echo admin_url('admin.php?page=oQeyVideo'); ?>">
<table width="880" border="0" cellspacing="0" cellpadding="0" style="margin:10px;">
  <tr>
    <td height="26" colspan="3">&nbsp;<?php _e('Add youtube embed code', 'oqey-gallery'); ?> </td>
    </tr>
  <tr>
    <td width="614"><input name="youtubevideoname" type="text" id="youtubevideoname" style="width:600px;"/></td>
    <td width="266" colspan="2"><input class="button" type="submit" name="saveYoutTubevideo" id="saveYoutTubevideo" value="<?php _e('Add video', 'oqey-gallery'); ?>" /></td>
    </tr>
</table>
</form>
</div>
		...
		$video_id=$video->id;
			...
			if(!empty($videosDesc)){
				foreach($videosDesc as $vdesc){
					?>
					<h4><u><?php echo $vdesc->lang; ?></u></h4>
		</div>
      <form action="" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general<?php echo $vdesc->wp_video_desc_id; ?>-en" class="en">
			...
			<tr>
			 ...
			 <td width="502"><span alt="<?php echo $vdesc->lang; ?>" class="dblclick" id="video_title_<?php echo $vdesc->wp_video_desc_id; ?>"><?php echo $vdesc->wp_title; ?></span></td>
			</tr>
			<tr>
...
jQuery(".vedit").click(function(){ 
 ...
 jQuery(".videoimage").click(function(){ //scan server for images 
   ...
   jQuery(".dblclick").editable( ajaxurl, { 
			...
			return { "action": 'oQeyEditVideoTitle', }; }
   });
