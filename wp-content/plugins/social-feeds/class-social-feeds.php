...
class SocialFeeds {
  ...
  public function getData() {
    ...
    $sql = "SELECT * FROM fhg_fh_social_feeds";
  ...
  public function socialicons($result) {
    echo '<div class="slide-out-div1"><img class="ajaxloader" alt="social feed-instagram" src="' . plugin_dir_url(__FILE__) . '/images/ajax-loader.gif" /><a class="handle1" href="#"> </a> <nav class="social">
           <div class="socicons"><ul class="nav-social">';

    if (isset($result->facebook_usr_id) && !empty($result->facebook_usr_id)) {
      echo '<li><a href="#" class="facebook" onclick="return false;"><i class="fa fa-facebook"></i></a></li>';
    }
    if (isset($result->twiter_screen_name) && !empty($result->twiter_screen_name)) {
      echo '<li><a href="#" class="twitter" onclick="return false;"><i class="fa fa-twitter"></i></a></li>';
    }

    echo '<li><a href="#" class="youtube" onclick="return false;"><i class="fa fa-youtube"></i></a></li>';

    if (isset($result->linked_in_code) && !empty($result->linked_in_code)) {
      echo '<li><a href="#" class="linkedin" onclick="return false;"><i class="fa fa-linkedin"></i></a></li>';
    }
    if (!empty($result->instagram_usr_id) || !empty($result->instagram_access_tkn)) {
      echo '<li><a href="#" class="instagram" onclick="return false;"><i class="fa fa-instagram"></i></a></li>';
    }
  ...
  function social_facebook($facebook_id, $fb_access_tkn, $nocount) {
    ...
    //$this->getFacebookFollow();
    $data = array();
    echo '<div class="container"><div class="soc-content"><div class="feed-facebook"><div class="fb-like-box">';
    echo '<p><div class="fb-follow" data-href="https://www.facebook.com/MyFinanceHouse/" data-width="200" data-height="200" data-layout="button" data-show-faces="true"></div></p>';
    foreach ($fbdata->data as $post) {
      ...
      echo '<a href="' . $data['post_link'] . '" target="_blank"><i class="fa fa-facebook"></i>Follow</a></div>';
...
// <script type="IN/CompanyProfile" data-id="448684" data-format="inline"></script>
  ...
  public function getYoutubeSubscribe(){ ?>
   ...
   echo '<div class="g-ytsubscribe" data-channelid="UCL54wvyyHcOi4Dxmy-9pXGA" data-layout="default" data-theme="dark" data-count="hidden" data-onytevent="onYtEvent"></div>';
  ...
  public function youtube($youtube, $vidcount, $page) {
    ...
    //echo count($data);
    echo '<div class="getYoutube" style="display:none;">';
    // $this->getYoutubeSubscribe();
    foreach ($data as $vid) {
      $url = $vid->id->videoId;
      //echo $url;
      ...
      //print_r($vid);
      if ($page == 'home') {
        echo '<div class="feed-youtube" >';
        echo '<iframe width="200" height="150" src="https://www.youtube.com/embed/' . $url . '?autoplay=0" frameborder="0"></iframe>';
        echo '<a href="https://www.youtube.com/watch?v='.$url.'" target="_blank"><i class="fa fa-youtube"></i>Follow</a>';
        echo '</div>';
      } else if ($page == 'social') {
        ...
        echo '<div class="caption">';
        echo '<p><a href="javascript:void(0);" onclick="return false;" class="btn btn-primary" role="button">Youtube</a><a href="' . $post->link . '" class="btn btn-primary" role="button">Read More</a></p>';
        echo '</div></div></div>';
      }
      $count++;
      if ($count == $vidcount) {
 ...
 $result = $this->instagramauth($insta_usr_id, $insta_tkn);
    ...
    foreach ($result->data as $instagram) {
      ...
      $desc = wp_trim_words($instagram->caption->text,15);
      // echo "<pre>";
      // print_r($instagram);
      echo '<div class="instagramdata"><a class="group" rel="group1" href="' . $url . '"><img style="display:block;position:relative;z-index:999;height:80px; width:80px; margin-right:10px; border:2px solid #309ccc;" src="' . $imge . '"></a>';
      echo '<h1>'.$name.'</h1>';
      echo '<p>'.$desc.'</p>';
      echo '<a target="_blank" href="'.$url.'" class="tfollowLink"><i class="fa fa-instagram"></i>Follow</a>';
      echo '</div>';
      $instcount++;
      if ($instcount == 2) {
        break;
